...
import type {
  SecureDocsHeroContent,
  SecureDocsComplianceContent,
  SecureDocsProtocolContent,
} from '../admin/sections/services-secure-docs/types';
...
interface SecureBoxProps { Icon: LucideIcon; title: string; content: string; delay?: number; }
...
const SecureDocsHero: React.FC = () => {
  const { data, loading } = useSectionContent<SecureDocsHeroContent>('securedocs_hero');
    ...
    <section className="w-full min-h-[100svh] flex items-center justify-center pt-20 md:pt-32 pb-12 px-5 md:px-12 bg-white" style={{ fontFamily: '"Manrope", sans-serif' }}>
      <div className="max-w-[1200px] mx-auto w-full flex flex-col lg:flex-row items-center gap-8 lg:gap-20">
        <div className="w-full lg:w-1/2 flex flex-col items-start text-start order-1">
          ...
          <Reveal delay={300}><p className="text-gray-600 font-light leading-relaxed text-[14px] md:text-[18px] mb-6 md:mb-10 max-w-[480px]">{data.description}</p></Reveal>
...
const ComplianceDetails: React.FC = () => {
  ...
  const { data, loading } = useSectionContent<SecureDocsComplianceContent>('securedocs_compliance');
...
const LEGAL_ICONS: LucideIcon[] = [Gavel, Landmark, FileText];

const LegalProtocol: React.FC = () => {
  const { data, loading } = useSectionContent<SecureDocsProtocolContent>('securedocs_protocol');
  if (loading || !data) return null;
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
          {data.cards.map((card, i) => (
            <SecureBox key={i} delay={(i + 1) * 100} Icon={LEGAL_ICONS[i] ?? Gavel} title={card.title} content={card.content} />
          ))}
        </div>
      </div>
    </section>
