...
import type {
  SecureMailHeroContent,
  SecureMailDetailsContent,
  SecureMailProtocolContent,
} from '../admin/sections/services-secure-mail/types';
...
interface GreenBoxProps { Icon: LucideIcon; title: string; content: string; delay?: number; }
...
const SecureMailHero: React.FC = () => {
  const { data, loading } = useSectionContent<SecureMailHeroContent>('securemail_hero');
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-start text-start order-1">
          ...
          <div className="hidden lg:flex flex-row gap-3 w-full sm:w-auto">
            <a href={data.btnPrimaryUrl || '/'} className="bg-[#36B936] hover:bg-[#2da12d] transition-all duration-300 text-white rounded-full px-8 py-3.5 text-[13px] font-medium tracking-wide shadow-lg active:scale-95 flex items-center gap-2">
              <span className="font-extralight text-base leading-none">+</span><span>{data.btnPrimaryLabel}</span>
            </a>
            <a href={data.btnSecondaryUrl || '/'} className="bg-white border border-gray-200 hover:border-[#36B936] transition-all duration-300 text-[#0A4D26] rounded-full px-8 py-3.5 text-[13px] font-medium tracking-wide active:scale-95 flex items-center gap-2">
              <span className="font-extralight text-base leading-none">+</span><span>{data.btnSecondaryLabel}</span>
            </a>
          </div>
        </div>
        ...
        <div className="flex lg:hidden flex-col gap-3 w-full order-3">
          <Reveal delay={600}>
            <a href={data.btnPrimaryUrl || '/'} className="bg-[#36B936] hover:bg-[#2da12d] transition-all duration-300 text-white rounded-full w-full py-4 text-[14px] font-medium tracking-wide shadow-lg active:scale-95 flex items-center justify-center gap-2">
              <span className="font-extralight text-lg leading-none">+</span><span>{data.btnPrimaryLabel}</span>
            </a>
          </Reveal>
          <Reveal delay={700}>
            <a href={data.btnSecondaryUrl || '/'} className="bg-white border border-gray-200 hover:border-[#36B936] transition-all duration-300 text-[#0A4D26] rounded-full w-full py-4 text-[14px] font-medium tracking-wide active:scale-95 flex items-center justify-center gap-2">
              <span className="font-extralight text-lg leading-none">+</span><span>{data.btnSecondaryLabel}</span>
            </a>
          </Reveal>
        </div>
      </div>
...
const MailDetails: React.FC = () => {
  ...
  const { data, loading } = useSectionContent<SecureMailDetailsContent>('securemail_details');
      ...
      <div className="max-w-[1200px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-24 items-start">
          <div className="lg:col-span-5 flex flex-col">
            <Reveal>
              <h2 className="text-[#0A4D26] text-[2rem] md:text-[3.5rem] font-light leading-[1.1] mb-5 text-start whitespace-pre-line">{data.title}</h2>
...
const MAIL_ICONS: LucideIcon[] = [Mail, Building2, EyeOff];

const MailSecurityProtocol: React.FC = () => {
  const { data, loading } = useSectionContent<SecureMailProtocolContent>('securemail_protocol');
  if (loading || !data) return null;
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-8">
          {data.cards.map((card, i) => (
            <GreenBox key={i} delay={(i + 1) * 100} Icon={MAIL_ICONS[i] ?? Mail} title={card.title} content={card.content} />
          ))}
        </div>
      </div>
    </section>
