...
import type { IntlHeroContent } from './types';
...
const IntlHeroEditor: React.FC = () => {
  ...
  const setField = (field: keyof IntlHeroContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  const setSharedField = (field: keyof IntlHeroContent, value: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
          <ImageUploader label="Desktop Background" currentUrl={contentEn.desktop_image_url} folder="intl" onUploaded={(url) => setSharedField('desktop_image_url', url)} />
        </div>
        <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
          <ImageUploader label="Mobile Background" currentUrl={contentEn.mobile_image_url} folder="intl" onUploaded={(url) => setSharedField('mobile_image_url', url)} />
        </div>
      </div>
