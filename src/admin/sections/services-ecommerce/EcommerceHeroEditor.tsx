...
import type { EcommerceHeroContent } from './types';
...
const EcommerceHeroEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<EcommerceHeroContent>('ecommerce_hero');
  ...
  const setField = (field: keyof EcommerceHeroContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  const setSharedField = (field: keyof EcommerceHeroContent, value: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
          <ImageUploader label="Desktop Background" currentUrl={contentEn.desktop_image_url} folder="ecommerce" onUploaded={(url) => setSharedField('desktop_image_url', url)} />
        </div>
        <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
          <ImageUploader label="Mobile Background" currentUrl={contentEn.mobile_image_url} folder="ecommerce" onUploaded={(url) => setSharedField('mobile_image_url', url)} />
        </div>
      </div>
...
export default EcommerceHeroEditor;