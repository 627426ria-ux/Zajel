...
import type { EcommerceDetailsContent } from './types';
...
const EcommerceDetailsEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<EcommerceDetailsContent>('ecommerce_details');
  ...
  const setField = (field: keyof EcommerceDetailsContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  const setSharedField = (field: keyof EcommerceDetailsContent, value: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
        <ImageUploader
          ...
          folder="ecommerce"
          onUploaded={(url) => { setContentEn(p => p ? { ...p, image_url: url } : p); setContentAr(p => p ? { ...p, image_url: url } : p); }}
        />
      </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div><label className="text-xs text-gray-500 block mb-1">Image Alt Text</label><input type="text" value={active.imgAlt || ''} onChange={e => setField('imgAlt', e.target.value)} className={inputClass} /></div>
      </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="text-xs text-gray-500 block mb-1">Contact Button Label</label><input type="text" value={active.btnContact || ''} onChange={e => setField('btnContact', e.target.value)} className={inputClass} /></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs text-gray-500 block mb-1">Quote Button Link</label><input type="text" value={contentEn.btnQuoteUrl || ''} onChange={e => setSharedField('btnQuoteUrl', e.target.value)} className={inputClass} placeholder="/quotation" /></div>
          <div><label className="text-xs text-gray-500 block mb-1">Contact Button Link</label><input type="text" value={contentEn.btnContactUrl || ''} onChange={e => setSharedField('btnContactUrl', e.target.value)} className={inputClass} placeholder="/contact" /></div>
        </div>
      </div>
    </div>
  );
};
export default EcommerceDetailsEditor;