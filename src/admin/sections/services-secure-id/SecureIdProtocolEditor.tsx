...
const SecureIdProtocolEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<SecureIdProtocolContent>('secureid_protocol');
  ...
  const setField = (field: keyof SecureIdProtocolContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  ...
  const ICON_LABELS = ['Shield Check', 'User Check', 'Fingerprint'];
...
export default SecureIdProtocolEditor;