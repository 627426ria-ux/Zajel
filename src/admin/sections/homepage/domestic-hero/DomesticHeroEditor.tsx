...
import type { DomesticHeroContent } from './types';
...
const DomesticHeroEditor: React.FC = () => {
  ...
  const setField = (field: keyof DomesticHeroContent, value: string) =>
    setActive((prev) => (prev ? { ...prev, [field]: value } : prev));

  const setSharedField = (field: keyof DomesticHeroContent, value: string) => {
    ...
    <div className="space-y-6">
      ...
      <section>
        <h4 className="text-xs font-normal text-gray-400 uppercase tracking-wider mb-3">
          Media &amp; Call to Action <span className="normal-case text-gray-300">— shared across languages</span>
        </h4>
        <div className="bg-white rounded-xl border border-[#E8E7E2] p-5 space-y-4">
          <ImageUploader
            label="Hero Background Image"
            currentUrl={contentEn.image_url}
            folder="domestic-hero"
          ...
          <div>
            ...
            <input type="text" value={contentEn.ctaLink || ''} onChange={(e) => setSharedField('ctaLink', e.target.value)} placeholder="/quotation" className={inputClass} />
          </div>
        </div>
      </section>
      ...
      <section dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        ...
        <div className="bg-white rounded-xl border border-[#E8E7E2] p-5 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={active.button} onChange={(e) => setField('button', e.target.value)} className={inputClass} />
          </div>
        </div>
      </section>
    </div>
...
export default DomesticHeroEditor;