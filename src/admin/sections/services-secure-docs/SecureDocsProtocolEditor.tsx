...
import type { SecureDocsProtocolContent } from './types';
...
const GovProtocolEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<SecureDocsProtocolContent>('securedocs_protocol');
  ...
  const setField = (field: keyof SecureDocsProtocolContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  ...
  const ICON_LABELS = ['Gavel', 'Landmark', 'File Text'] // icons stay hardcoded on the frontend, shown here for reference only
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-5" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <h4 className="text-xs font-bold text-gray-400 uppercase">Cards (3) — icons are fixed in code, not editable here</h4>
        {[0, 1, 2].map((i) => (
...
export default GovProtocolEditor;