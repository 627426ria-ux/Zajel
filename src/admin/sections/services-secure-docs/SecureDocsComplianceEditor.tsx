...
import type { SecureDocsComplianceContent } from './types';
...
const GovComplianceEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<SecureDocsComplianceContent>('securedocs_compliance');
  ...
  const setField = (field: keyof SecureDocsComplianceContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div><label className="text-xs text-gray-500 block mb-1">Title (part 1, black)</label><input type="text" value={active.title1 || ''} onChange={e => setField('title1', e.target.value)} className={inputClass} /></div>
...
export default GovComplianceEditor;