...
import type { GovProtocolContent } from './types';
...
const GovProtocolEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<GovProtocolContent>('gov_protocol');
  ...
  const setField = (field: keyof GovProtocolContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  ...
  const ICON_LABELS = ['Landmark', 'File Check', 'History']; // icons stay hardcoded on the frontend, shown here for reference only
