...
import type { GovHeroContent } from './types';
...
const GovHeroEditor: React.FC = () => {
  ...
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<GovHeroContent>('gov_hero');
  ...
  const setField = (field: keyof GovHeroContent, value: string) => {
  ...
  const setSharedField = (field: keyof GovHeroContent, value: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
        <ImageUploader label="Hero Image" currentUrl={contentEn.image_url} folder="government" onUploaded={(url) => setSharedField('image_url', url)} />
        <div className="mt-4">
