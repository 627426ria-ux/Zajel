...
import type { GovComplianceContent } from './types';
...
const GovComplianceEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<GovComplianceContent>('gov_compliance');
  ...
  const setField = (field: keyof GovComplianceContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
