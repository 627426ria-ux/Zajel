...
const SecureMailDetailsEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<SecureMailDetailsContent>('securemail_details');
  ...
  const setField = (field: keyof SecureMailDetailsContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
        <div><label className="text-xs text-gray-500 block mb-1">Title</label><textarea value={active.title || ''} onChange={e => setField('title', e.target.value)} rows={2} className={inputClass} /></div>
...
export default SecureMailDetailsEditor;