...
const SecureMailHeroEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<SecureMailHeroContent>('securemail_hero');
  ...
  const setField = (field: keyof SecureMailHeroContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  const setSharedField = (field: keyof SecureMailHeroContent, value: string) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2]">
        <ImageUploader label="Hero Image" currentUrl={contentEn.image_url} folder="secure-mail" onUploaded={(url) => setSharedField('image_url', url)} />
        <div className="mt-4">
...
export default SecureMailHeroEditor;