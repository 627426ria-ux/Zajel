...
import type { FreightDetailsContent } from './types';
...
const FreightDetailsEditor: React.FC = () => {
  const { contentEn, contentAr, setContentEn, setContentAr, enabled, setEnabled, loading, saving, dirty, save } =
    useBilingualSectionContent<FreightDetailsContent>('freight_details');
  ...
  const setField = (field: keyof FreightDetailsContent, value: string) => setActive(prev => prev ? { ...prev, [field]: value } : prev);
  const setSharedField = (field: keyof FreightDetailsContent, value: string) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-[#E8E7E2] space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="text-xs text-gray-500 block mb-1">Track Button Link</label><input type="text" value={contentEn.btnTrackUrl || ''} onChange={e => setSharedField('btnTrackUrl', e.target.value)} className={inputClass} placeholder="/track-shipment" /></div>
        </div>
      </div>
    </div>
  );
};
export default FreightDetailsEditor;