...
import type { FreightDetailsContent } from '../admin/sections/services-freight/types';

interface PageSectionRow {
  id: string;
  enabled: boolean;
  content_en: FreightDetailsContent | string;
  content_ar: FreightDetailsContent | string;
}

const FreightDetails: React.FC = () => {
  const { i18n } = useTranslation();
  const isRtl = i18n.language === 'ar';
  const [data, setData] = useState<FreightDetailsContent | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!error && section?.enabled) {
        ...
        const content: FreightDetailsContent | null =
            ...
            <>
              <div className={`flex flex-col p-6 sm:p-8 lg:absolute lg:bottom-12 lg:p-0 z-10 order-1 lg:order-none ${isRtl ? 'lg:right-10 lg:left-10' : 'lg:left-10 lg:right-10'}`}>
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  ...
                  >
                    <span className="text-xl font-extralight leading-none">+</span>
                    <span className="font-medium tracking-wide">{data.btnTrack}</span>
                  </Link>
                </div>
              </div>
              <div className="relative w-full h-[220px] sm:h-[300px] lg:h-[600px] order-2 lg:order-none">
                <img src={data.image_url} alt="Zajel" className="w-full h-full object-cover" />
