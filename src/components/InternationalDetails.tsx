...
import type { IntlDetailsContent } from '../admin/sections/services-intl/types';

interface PageSectionRow {
  id: string;
  enabled: boolean;
  content_en: IntlDetailsContent | string;
  content_ar: IntlDetailsContent | string;
}

const InternationalDetails: React.FC = () => {
  const { i18n } = useTranslation();
  const isRtl = i18n.language === 'ar';
  const [data, setData] = useState<IntlDetailsContent | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (!error && section?.enabled) {
        ...
        const content: IntlDetailsContent | null =
