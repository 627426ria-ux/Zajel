...
const testimonials = [
  {
    text: "Excellent service saved my money and very convenient, me savers quickly book from their website, they will collect it and deliver Quick service",
    name: "Saud Esaf",
  ...
  {
    text: "The team was incredibly professional. Everything handled with care and delivered ahead of schedule. Will definitely recommend to everyone.",
    name: "Ravi Menon",
    role: "Software Engineer",
    image: "/image copy 6.png"
  },
  {
    text: "Booking was seamless and pickup was right on time. Great communication throughout the entire process. Highly recommend their services.",
    name: "Fatima Zahra",
    role: "Marketing Lead",
    image: "/image copy 6.png"
  }
...
const TestimonialCard = ({ item }: { item: typeof testimonials[0] }) => (
  <div style={{
    background: '#FFFFFF',
    border: '1.2px solid rgba(43, 167, 53, 0.18)',
    borderRadius: '24px',
    ...
    <p style={{ color: '#064423', fontSize: '14px', fontWeight: 400, lineHeight: '1.78', margin: '0 0 28px 0', flex: 1, opacity: 0.92 }}>
      {item.text}
    </p>
...
const ArrowBtn = ({ direction, disabled, onClick }: { direction: 'left' | 'right'; disabled: boolean; onClick: () => void }) => (
  <button
    ...
    style={{
      ...
      height: '40px',
      borderRadius: '50%',
      border: `1px solid ${disabled ? 'rgba(6,68,35,0.1)' : 'rgba(6,68,35,0.18)'}`,
      background: '#fff',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      cursor: disabled ? 'not-allowed' : 'pointer',
      color: disabled ? 'rgba(6,68,35,0.2)' : 'rgba(6,68,35,0.4)',
      transition: 'border-color .2s, color .2s',
      padding: 0,
    }}
    onMouseEnter={e => { if (!disabled) { (e.currentTarget as HTMLButtonElement).style.borderColor = '#2BA735'; (e.currentTarget as HTMLButtonElement).style.color = '#2BA735'; }}}
    onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = disabled ? 'rgba(6,68,35,0.1)' : 'rgba(6,68,35,0.18)'; (e.currentTarget as HTMLButtonElement).style.color = disabled ? 'rgba(6,68,35,0.2)' : 'rgba(6,68,35,0.4)'; }}
  >
...
const Testimonials: React.FC = () => {
  ...
  useEffect(() => {
    setIdx(prev => Math.min(prev, Math.max(0, testimonials.length - cardsPerView)));
  }, [cardsPerView]);
  ...
  const visible = testimonials.slice(idx, idx + cardsPerView);
    ...
    <section style={{ width: '100%', background: '#FDFDFD', padding: 'clamp(48px, 8vw, 80px) clamp(20px, 5vw, 48px)', fontFamily: '"Manrope", sans-serif' }}>
      <div style={{ maxWidth: '1260px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '48px' }}>
          <h2 style={{ color: '#064423', fontSize: 'clamp(1.8rem, 3vw, 2.8rem)', fontWeight: 300, letterSpacing: '-0.5px', lineHeight: 1.1, margin: '0 0 10px 0' }}>
            Testimonials
          </h2>
          <p style={{ color: 'rgba(6,68,35,0.45)', fontSize: '13px', margin: 0 }}>
            Latest feedback from our valued customers
          </p>
        </div>

        {/* CAROUSEL ROW — arrows only on tablet/desktop */}
        <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? 0 : '16px' }}>
          ...
          <div style={{ overflow: 'hidden', flex: 1 }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            <AnimatePresence mode="wait" custom={slideDir}>
              <motion.div
                key={`${idx}-${cardsPerView}`}
                custom={slideDir}
                initial={{ opacity: 0, x: slideDir * 60 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: slideDir * -60 }}
                transition={{ duration: 0.4, ease: [0.25, 0.46, 0.45, 0.94] }}
                style={{ display: 'grid', gridTemplateColumns: gridCols, gap: '16px' }}
              >
                {visible.map((item, i) => (
                  <TestimonialCard key={`${idx}-${i}`} item={item} />
                ))}
              </motion.div>
            </AnimatePresence>
          </div>
        ...
        {/* DOTS ROW — arrows inline on mobile */}
            ...
            {Array.from({ length: maxIdx + 1 }).map((_, i) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: '4px',
                  border: 'none',
                  background: i === idx ? '#2BA735' : 'rgba(6,68,35,0.15)',
                  cursor: 'pointer',
                  padding: 0,
