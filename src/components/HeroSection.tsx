...
const InternationalIcon = () => (
  <svg width="26" height="28" viewBox="0 0 40 28" fill="none">
...
const DOT_COUNT = 4; // QUICK_ACTIONS length
...
const StatsBar: React.FC = () => {
  ...
  const STATISTICS = [
    ...
    { value: '45M+',  labelKey: 'hero.stats.shipments' },
    { value: '200+',  labelKey: 'hero.stats.countries' },
    { value: '500K+', labelKey: 'hero.stats.destinations' },
  ];
...
const HeroSection: React.FC = () => {
  ...
  // ── Slides — titles & descs from translation keys ──
  const HERO_SLIDES = [
    { titleKey: 'hero.slides.0.title', descKey: 'hero.slides.0.desc', bgDesktop: '/ChatGPT Image May 14, 2026, 01_14_25 PM.png', bgMobile: '/ChatGPT Image May 14, 2026, 01_13_31 PM.png' },
    { titleKey: 'hero.slides.1.title', descKey: 'hero.slides.1.desc', bgDesktop: '/ChatGPT Image May 12, 2026, 02_58_21 AM.png', bgMobile: '/ChatGPT Image May 12, 2026, 02_58_25 AM.png' },
    { titleKey: 'hero.slides.2.title', descKey: 'hero.slides.2.desc', bgDesktop: '/ChatGPT Image May 12, 2026, 02_30_58 AM.png', bgMobile: '/ChatGPT Image May 12, 2026, 02_26_18 AM.png' },
    { titleKey: 'hero.slides.3.title', descKey: 'hero.slides.3.desc', bgDesktop: '/ChatGPT Image May 14, 2026, 01_24_32 AM.png', bgMobile: '/ChatGPT Image May 14, 2026, 01_30_01 AM.png' },
    { titleKey: 'hero.slides.4.title', descKey: 'hero.slides.4.desc', bgDesktop: '/ChatGPT Image May 14, 2026, 02_22_11 AM.png', bgMobile: '/ChatGPT Image May 14, 2026, 02_23_35 AM.png' },
    { titleKey: 'hero.slides.5.title', descKey: 'hero.slides.5.desc', bgDesktop: '/ChatGPT Image May 15, 2026, 05_28_51 AM.png', bgMobile: '/ChatGPT Image May 15, 2026, 05_30_17 AM.png' },
  ];

  const QUICK_ACTIONS = [
    { labelKey: 'hero.actions.track.label',         fullLabelKey: 'hero.actions.track.fullLabel',         descKey: 'hero.actions.track.desc',         path: '/track',               Icon: TrackIcon,         iconBg: '#2BA735' },
    { labelKey: 'hero.actions.quotation.label',     fullLabelKey: 'hero.actions.quotation.fullLabel',     descKey: 'hero.actions.quotation.desc',     path: '/quotation',           Icon: QuotationIcon,     iconBg: '#5B3C8C' },
    { labelKey: 'hero.actions.domestic.label',      fullLabelKey: 'hero.actions.domestic.fullLabel',      descKey: 'hero.actions.domestic.desc',      path: '/domestic-courier',    Icon: DomesticIcon,      iconBg: '#C8922A' },
    { labelKey: 'hero.actions.international.label', fullLabelKey: 'hero.actions.international.fullLabel', descKey: 'hero.actions.international.desc', path: '/international-courier', Icon: InternationalIcon, iconBg: '#C0392B' },
  ];
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setTimeout(() => {
        ...
        setSlidePhase('enter');
        setTextKey(k => k + 1);
        setTimeout(() => { setPrevHeroIdx(null); setSlidePhase('idle'); }, 1400);
      }, 700);
    }, 5000);
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes kbZoomOut { from { transform: scale(1.1); } to { transform: scale(1.0); } }
        .kb-0, .kb-1, .kb-2, .kb-3, .kb-4 { animation: kbZoomOut 5.7s ease-out both; }
        ...
        .slide-text-title { animation: textReveal 0.9s cubic-bezier(0.16, 1, 0.3, 1) forwards; opacity: 0; }
        .slide-text-desc  { animation: subtitleReveal 0.85s 0.18s cubic-bezier(0.16, 1, 0.3, 1) forwards; opacity: 0; }

        .hero-bg-entering { animation: crossFadeIn  0.85s cubic-bezier(0.4, 0, 0.2, 1) forwards; }
        .hero-bg-exiting  { animation: crossFadeOut 0.7s  cubic-bezier(0.4, 0, 0.2, 1) forwards; }
        ...
        .slide-progress { height: 2px; background: rgba(255,255,255,0.35); border-radius: 99px; overflow: hidden; }
        .slide-progress-bar { height: 100%; background: rgba(255,255,255,0.9); border-radius: 99px; animation: progressBar 5s linear forwards; }
      ...
      >
        ...
        {HERO_SLIDES.map((slide, idx) => {
          ...
          const opacity =
            ...
            isExiting && slidePhase === 'enter' ? 0         : 0;

          return (
            <div key={idx} className={`absolute inset-0 w-full h-full overflow-hidden ${fadeClass}`}
              style={{ zIndex: isActive ? 2 : isExiting ? 1 : -1, opacity }}>
              <div key={`mob-bg-${idx}-${textKey}`}
        ...
        <div className="mobile-only absolute inset-0 pointer-events-none"
        ...
        <div className="desktop-only absolute inset-0 pointer-events-none"
        ...
        <div className="mobile-only flex-col relative min-h-[100svh]" style={{ zIndex: 10 }}>
          <div className="px-5 pt-24">
            <form onSubmit={handleTrackSubmit}
              ...
              <input type="text" placeholder={t('hero.track.placeholder')}
                className="flex-1 px-3.5 outline-none text-[13px] text-gray-700 bg-transparent placeholder:text-gray-400"
                dir={isRtl ? 'rtl' : 'ltr'} />
              <button type="submit"
                className="bg-[#15A744] text-white px-4 py-2.5 rounded-full text-[13px] font-semibold flex items-center gap-1.5 shrink-0">
                {t('hero.track.button')} <Search size={12} strokeWidth={2.5} />
              </button>
            </form>
          </div>
          ...
          <div className="px-5 pb-8 flex flex-col gap-5">
            <div key={`mob-${textKey}`} className="min-h-[110px]">
              <h1 className="slide-text-title font-light text-white leading-[1.08] tracking-tight mb-3 whitespace-pre-line"
                style={{ fontSize: 'clamp(2.6rem, 11vw, 3.5rem)' }}>
                {t(HERO_SLIDES[heroIdx].titleKey)}
              </h1>
              <p className="slide-text-desc text-white/90 font-normal leading-snug whitespace-pre-line"
                style={{ fontSize: 'clamp(14px, 3.8vw, 17px)' }}>
                {t(HERO_SLIDES[heroIdx].descKey)}
              </p>
            </div>
            ...
            <div className="hf5"><StatsBar /></div>
          </div>
        </div>
        ...
        <main className="desktop-only relative h-screen flex flex-col justify-between px-6 md:px-10 lg:px-14 pt-44 pb-10" style={{ zIndex: 10 }}>
          <div>
            <form onSubmit={handleTrackSubmit}
              ...
              dir={isRtl ? 'rtl' : 'ltr'}>
              <label htmlFor="dt-track" className="sr-only">{t('hero.track.placeholder')}</label>
              <input id="dt-track" type="text" placeholder={t('hero.track.placeholder')}
                className="flex-1 px-3 outline-none text-[14px] text-[#0A4D26] font-medium bg-transparent placeholder:text-gray-400" />
              <button type="submit"
                className="bg-[#15A744] hover:bg-[#118f39] transition-all duration-300 text-white px-7 py-3 rounded-full text-[13.5px] font-bold flex items-center gap-2 shadow-[0_4px_14px_rgba(21,167,68,0.25)] hover:shadow-[0_6px_20px_rgba(21,167,68,0.4)]">
                {t('hero.track.button')} <Search size={14} strokeWidth={2.5} />
              </button>
            </form>

            <div key={`dt-${textKey}`} className="min-h-[200px]">
              <h1 className="slide-text-title font-light text-[#0A4D26] leading-[1.05] tracking-tight mb-5 whitespace-pre-line"
                style={{ fontSize: 'clamp(3.5rem, 6.5vw, 6.5rem)' }}>
                {t(HERO_SLIDES[heroIdx].titleKey)}
              </h1>
              <p className="slide-text-desc text-[#0A4D26] font-normal max-w-sm leading-snug whitespace-pre-line"
          ...
          <div className="flex flex-row justify-between items-end w-full gap-10">
            <div className="hf4 flex flex-col gap-3.5 shrink-0">
              <Link to={action.path} className={`qa-card ${isAnimating ? (animDir === 'right' ? 'qa-sr' : 'qa-sl') : ''}`}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[#004E09] text-[13px] font-semibold leading-tight mb-1">{t(action.fullLabelKey)}</p>
                  <p className="text-[#004E09]/50 text-[11px] leading-snug line-clamp-2">{t(action.descKey)}</p>
                </div>
                <div className="qa-arrow"><ChevronRight size={12} strokeWidth={2} /></div>
              </Link>
              <div className="flex items-center gap-2 pl-0.5">
                <button onClick={prev} aria-label={t('hero.nav.prev')} className="qa-nav"><ChevronLeft size={12} strokeWidth={2} /></button>
                ...
                <button onClick={next} aria-label={t('hero.nav.next')} className="qa-nav"><ChevronRight size={12} strokeWidth={2} /></button>
              </div>
            </div>
            ...
            <div className="hf5 flex-1 min-w-0 flex flex-col items-end gap-5">
              ...
              <p className="text-right text-white/80 drop-shadow-sm leading-relaxed"
